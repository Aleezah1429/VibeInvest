...
All endpoints live on the FastAPI backend ([backend/app/main.py](../../backend/app/main.py)). Base URL: `http://127.0.0.1:8000` in dev, the Railway domain in prod. Content-Type: `application/json` unless noted. Authenticated endpoints need `Authorization: Bearer <token>`. CORS: `allow_origins=["*"]`.
