...
- `market_saturation_score >= 9` (market is dead-saturated with 5+ funded incumbents), OR
- You found three or more direct competitors who are already serving the *exact* same
  audience with the *exact* same offering (i.e. no plausible wedge exists).

Soft pessimism ("market is competitive", "differentiation is unclear") is NOT a
kill_signal — leave those for `red_flags`. Reserve kill_signal for genuinely
non-viable. When in doubt, set `kill_signal: false`.
